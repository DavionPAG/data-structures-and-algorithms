...
const validateEmail = (email) => {
  ...
  return /^([\d\w]+\.[\w\d]+|[\d\w]+)@([\w\d]+).(com|net|org)$/gm.test(email);
};
...
const validatePhoneNumber = (phoneNumber) => {
  return /^(\(\d{3}\)(-|\s)|\d{3}(-|\s)|\d{3})(\d{3}(-|\s)|\d{3})(\d{4})$/g.test(phoneNumber);
};
...
const findTagNames = elements => {
  let pulled = elements.map(tags => {
    let check = tags.match(/((\/)((\d*\w*)))/g);

    if (check === true){
      let match = check.match(/((\/)((\d*\w*)))/g);
      return match;}
  });
  return pulled;
};
