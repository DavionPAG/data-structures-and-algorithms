...
const forEachTwoToThe = (arr) => {
  let forEachArr = [];
  arr.forEach(num => {
    forEachArr.push(Math.pow(2, num));
  });
  return forEachArr;
};
...
Write a function named mapTwoToThe that produces the same output as your forLoopTwoToThe function from challenge 2 and your forEachTwoToThe function from challenge 2, but uses map instead of a for loop or forEach.
------------------------------------------------------------------------------------------------ */
...
const extractStats = (arr) => {
  let statArr = arr.map(obj => {
    return (obj.stat.name, obj.baseStat);
  });
  return statArr;
};
